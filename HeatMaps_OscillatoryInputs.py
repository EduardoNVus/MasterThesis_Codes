...
def func_cos(X,t,params):
    ...
    derivadaX=-X+np.tanh(g*np.dot(W,X)) + R*np.cos(omega*t + theta)

    return derivadaX
#############################################################################
# Método de resolución de la EDO Runge-Kutta
@njit
def RK4(N, x0, h, tmax, params): 
    ...
    for tt in range(1,T): #loop on times, from 1 to T (the time 0 has already been set)
        k1=func_cos(X[:,tt-1], t[tt-1], params)
        k2=func_cos(X[:,tt-1]+h2*k1, t[tt-1]+h2, params)
        k3=func_cos(X[:,tt-1]+h2*k2, t[tt-1]+h2, params)
        k4=func_cos(X[:,tt-1]+h*k3, t[tt-1]+h, params)
...
############################################################
@njit
def Tmap(IgJ_array,JoJ_array,g,N,Xo,h,tmax,to,tm,R,omega):
        
        Z = np.random.normal(0,1,size = (N,N)) 
        M_matrix = np.zeros((len(IgJ_array),len(JoJ_array))) 
        C_matrix = np.zeros((len(IgJ_array),len(JoJ_array))) 
        for JoJ in JoJ_array: # -------------------------------------------------------------------------------
            for IgJ in IgJ_array: 
                ...
                t_sol,X_sol, T = RK4(N,Xo,h,tmax,(g,N,W,R,omega,theta)) 
                X_average = np.zeros(T)
                X_average2 = np.zeros(T)
                for i in range(T):
                    X_average[i] = np.mean(X_sol[:,i]) 
                    X_average2[i] = np.mean(X_sol[:,i]**2) 
                            
                Msini = np.where(t_sol >= to)[0][0]        
                Msfin = np.where(t_sol >= (to + tm))[0][0]   
                
                Ms = np.mean(X_average[Msini:Msfin]) 
...
########################################################################
@njit
def Average_Tmap(s,IgJ_array,JoJ_array,g,N,Xo,h,tmax,to,tm,R,omega):
    average_M = np.zeros((len(IgJ_array),len(JoJ_array)))
    average_C = np.zeros((len(IgJ_array),len(JoJ_array)))
    s0 = 0
    for realizations in range(s):
        M,C = Tmap(IgJ_array,JoJ_array,g,N,Xo,h,tmax,to,tm,R,omega)
        average_M += np.abs(M)
        average_C += C
...
N = 100                 # System size
Xo = np.random.normal(0,1,size = N)    # Initial condition

R = 1 # Amplitude of the external signal
omega = 1 # Frequency of the external signal
# Simulation parameters
s = 100  # Number of realizations  
tmax = 125 # Time of simulation
h = 0.1 # Time step
to = 100 # Initial time for the average
tm =20 # Time of the average
# ---------------------------------------------
...
# Exectute
for Ri in np.array([-0.5]):
    M,C = Average_Tmap(s,IgJ_array,JoJ_array,g,N,Xo,h,tmax,to,tm,Ri,omega)
    # Save
    fecha = datetime.datetime.now()
    np.savetxt(f"M_RandomSignal_R{Ri}",M,delimiter = ",",header = f"N = {N}, g = {g},1/gJ = ({IgJ_array[0]},{IgJ_array[-1]}), Jo/J = ({JoJ_array[0]},{JoJ_array[-1]}), h1/gJ = {hIgJ}, R = {Ri}, omega = {omega}| s = {s}, h = {h}, tmax = {tmax}, to = {to},tm = {tm} | Date: {str(fecha)}")
    np.savetxt(f"C_RandomSignal_R{Ri}",C, delimiter = ",",header = f"N = {N}, g = {g}, 1/gJ = ({IgJ_array[0]},{IgJ_array[-1]}), Jo/J = ({JoJ_array[0]},{JoJ_array[-1]}), hJo/J = {hJoJ}, R = {Ri}, omega = {omega}| s = {s}, h = {h}, tmax = {tmax}, to = {to},tm = {tm} | Date: {str(fecha)}")
##########################################################
# Read results
M_read_05 = np.loadtxt("M_RandomSignal_R0.5",delimiter = ",", comments="#")
C_read_05 = np.loadtxt("C_RandomSignal_R0.5",delimiter = ",", comments="#")

M_read_01 = np.loadtxt("M_RandomSignal_R0.1",delimiter = ",", comments="#")
C_read_01 = np.loadtxt("C_RandomSignal_R0.1",delimiter = ",", comments="#")

M_read_00 = np.loadtxt("M_RandomSignal_R0.0",delimiter = ",", comments="#")
C_read_00 = np.loadtxt("C_RandomSignal_R0.0",delimiter = ",", comments="#")

# Theoretical bifurcatin line between the chaotic phase and the persistent phase
def f(q, k):
    if q < 1e-10:
        return 0.0  
...
# Method to solve the equation q = f(q, k)
...
plt.title(r"M ($I_{0} = 0.5$)",fontsize = 15)
...
plt.xticks(fontsize = 13)
plt.yticks(fontsize = 13)
plt.title(r"q ($I_{0} = 0.5$)",fontsize = 15)
